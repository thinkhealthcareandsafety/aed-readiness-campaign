...
function migrateUnitScopedPhysicalAndReadiness() {
  ...
  ensureTemplateQuestion("Physical Status", "Is there any visible damage on AED (2) (cracks, swelling, or leakage)?", [
    ...
    { value: "no", label: "No — sound condition", points: 3, imageUrl: "/icons/damage-no.svg" },
  ]);
  ensureTemplateQuestion("Physical Status", "Is the AED (2) cabinet intact and accessible?", [
    { value: "yes", label: "Yes", points: 2, imageUrl: "/icons/cabinet-ok.svg" },
    { value: "no", label: "No", points: 0, imageUrl: "/icons/cabinet-bad.svg" },
  ]);
...
// regardless of how many AEDs a property actually reported. Matched by
// exact label + field_role currently NULL, so this only ever fills in the
// gap and never clobbers a role an admin might have set through the form
// builder since.
