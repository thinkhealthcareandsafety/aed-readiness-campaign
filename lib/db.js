...
function migrateScoringColumns() {
  // Same Railway build-vs-live-deployment race documented on
  // migrateSubmissionsTable's "prize" column above: two processes can both
  // see a column missing and both try to add it, and the loser gets a
  // genuine SQLITE_ERROR ("duplicate column name") that isn't actually a
  // problem — the column ends up added either way — so it's swallowed
  // rather than left to crash the build.
