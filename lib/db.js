...
const dbPath = process.env.AED_DB_PATH || path.join(DATA_DIR, "aed.db");
...
// Shared by every migration that adds a column to an already-existing
// table (CREATE TABLE IF NOT EXISTS doesn't retroactively add columns).
// The Railway build step and the live deployment both import this module
// against the same mounted volume (see openAndInitDb below) — if both
// happen to run a migration's check at nearly the same moment, both can
// see a column missing and both attempt to add it, and whichever loses
// the race gets a genuine SQLITE_ERROR ("duplicate column name"), not a
// lock-contention error busy_timeout can wait out. That's a builder crash
// from a race that isn't actually a problem — the column ends up added
// either way — so it's swallowed here rather than left to crash the build.
function addColumnIfMissing(table, column, ddl) {
  ...
  try {
    ...
    console.warn(`Could not add ${table}.${column} (likely already added by a concurrent process):`, err.message);
  }
}
...
if (!db) {
  ...
  migrateScoringColumns();
  migrateSubmissionsTable();
  migrateOptionImages();
  migrateOptionLabels();
  migrateEmergencyContactLabels();
  ...
  migrateOptionImagesRound3();
  migrateOptionImagesRound4();
  migrateAutoInspectionUnitFieldRoles();
  migrateTrainingFieldRoles();
  migrateExpiryDateLabels();
  migrateConfidenceScaleLabels();
  migrateRemoveDuplicateHotelOptions();
  migratePointRebalanceTo100();
  migrateBeepReferenceImages();
  migrateCabinetInaccessibleReferenceImage();
  migrateContactDetailsSection();
  migrateClearAllSectionNotes();
  migrateClearQuestionHints();
  migrateUpdateCabinetAccessibleImage();
  migratePointRebalanceTo200();
  migrateFixDuplicateSectionLetter();
  migrateNumberedSectionLetters();
  migrateDeliveryColumns();
  migrateCertificateNumbers();
  migrateServiceProviderLabel();
...
// `CREATE TABLE IF NOT EXISTS` doesn't retroactively add columns either —
// same reasoning as migrateSubmissionsTable above, for the three new
// scoring-engine flags a 200-point rework needed: a section whose per-unit
// questions should average together instead of sum (Physical Status,
// Readiness Alert, Expiry Status, Paediatric Readiness — each now a fixed
// weight regardless of how many AEDs are reported), a quantity/checkbox
// question whose selected options should average instead of sum (the
// AED-model question — several different models picked at once shouldn't
// out-score any one of them), and a text question whose score is derived
// from the AED serial number's own encoded manufacture date rather than a
// picked option (see serialAgePoints in genericScoring.js).
function migrateScoringColumns() {
  addColumnIfMissing("form_sections", "average_per_unit", `ALTER TABLE form_sections ADD COLUMN average_per_unit INTEGER NOT NULL DEFAULT 0`);
  addColumnIfMissing("form_questions", "average_across_selections", `ALTER TABLE form_questions ADD COLUMN average_across_selections INTEGER NOT NULL DEFAULT 0`);
  addColumnIfMissing("form_questions", "score_from_serial_age", `ALTER TABLE form_questions ADD COLUMN score_from_serial_age INTEGER NOT NULL DEFAULT 0`);
}
...
// "Ambulance (108 / local emergency line)" and "Loss Prevention / Security
// department" ran to 3 lines in the icon-card grid, next to 1-line options
// ("Doctor on call") in the exact same row — the uneven wrap was making the
// row's cards visibly different heights even with align-items: stretch,
// since a much longer label just eats more of the stretched card's own
// height. Shortened, with the ambulance detail moved to a sub-line instead
// of dropped. Matched by exact old label + value, so a re-run or an
// admin-customized label is left alone.
// lib/hotels.js used to carry the same 9 properties twice — once under a
// short "Courtyard <city>" name and again under the brand's actual full
// name "Courtyard by Marriott <city>" — because entries were added over
// time without checking for an existing short-form row first. Every other
// brand family in that list (Fairfield by Marriott, Four Points by
// Sheraton, ...) only ever appears in its one full form, so the full form
// is kept here too and the short-form duplicate removed. Matched by exact
// label on the "hotel" field-role question specifically (not a LIKE on
// question text, unlike the older migrations above) since that field has
// a stable, unique field_role — this only ever removes an option whose
// label is still exactly the original short form, so a property an admin
// has since renamed or already removed is left alone.
function migrateRemoveDuplicateHotelOptions() {
  const stmt = db.prepare(`
    DELETE FROM form_options WHERE label = ?
      AND question_id IN (SELECT id FROM form_questions WHERE field_role = 'hotel')
  `);
  for (const shortLabel of [
...
function migrateEmergencyContactLabels() {
  const stmt = db.prepare(`
    UPDATE form_options SET label = ?, sub = ?
    WHERE label = ? AND value = ?
      AND question_id IN (SELECT id FROM form_questions WHERE label LIKE '%emergency contact number%')
  `);
  for (const r of [
    { value: "ambulance", oldLabel: "Ambulance (108 / local emergency line)", newLabel: "Ambulance (108)", newSub: "Local emergency line" },
    { value: "lossPrevention", oldLabel: "Loss Prevention / Security department", newLabel: "Loss Prevention / Security", newSub: null },
  ]) {
    stmt.run(r.newLabel, r.newSub, r.oldLabel, r.value);
  }
}
...
function migrateUnitScopedPhysicalAndReadiness() {
  ...
  ensureTemplateQuestion("Physical Status", "Is the AED (2) cabinet intact and accessible?", [
    ...
    { value: "no", label: "No", points: 0, imageUrl: "/reference/aed-cabinet-inaccessible.jpg" },
  ]);
...
function migrateOptionImagesRound2() {
  ...
  const replacements = [
    ...
    { questionLabelLike: "%cabinet intact and accessible%", value: "no", newImage: "/reference/aed-cabinet-inaccessible.jpg" },
    ...
    { questionLabelLike: "%emergency accessories%", value: "gloves", newImage: "/accessories/gloves.jpg" },
  ];
  for (const r of replacements) {
...
// The "No" side of unit (1)'s battery/pads questions were still the
// original flat SVG icons (battery-bad.svg, pads-unsealed.svg) even after
// the "Yes" side got real reference photos in migrateOptionImages above —
// client-supplied real photos of a detached battery and the AED's own
// "connect pads" prompt screen. Same robust match-by-label+value pattern
// as Round 2 (not conditioned on the option's current image).
function migrateOptionImagesRound3() {
  const stmt = db.prepare(`
    UPDATE form_options SET image_url = ?
    WHERE value = ?
      AND question_id IN (SELECT id FROM form_questions WHERE label LIKE ?)
  `);
  for (const r of [
    { questionLabelLike: "%battery (1) installed correctly%", value: "no", newImage: "/reference/frx-battery-not-attached.jpg" },
    { questionLabelLike: "%pads (1) connected and sealed%", value: "no", newImage: "/reference/frx-pads-not-connected.jpg" },
  ]) {
    stmt.run(r.newImage, r.value, r.questionLabelLike);
  }
}

// The "visible damage" question's "Yes" side was a single generic flat
// icon (damage-yes.svg) on both unit (1) and (2) — replaced with a real
// 3-photo composite (cracked pads cover, cracked FRx front panel, cracked
// ZOLL case) so "damage" reads as concrete examples across brands rather
// than one abstract glyph. Not model-conditional like battery/pads above —
// damage can happen to any unit, so the same composite applies regardless
// of which model a unit reports.
function migrateOptionImagesRound4() {
  const stmt = db.prepare(`
    UPDATE form_options SET image_url = ?
    WHERE value = 'yes'
      AND question_id IN (SELECT id FROM form_questions WHERE label LIKE '%visible damage on AED%')
  `);
  stmt.run("/reference/damage-examples.jpg");
}
...
// Same missing-field_role gap as above, for the two AED Responder Training
// questions — needed so the combined recency+headcount status message (see
// trainingStatusMessage in AuditWizard.jsx) can look each question up by
// role instead of guessing at a specific question id.
function migrateTrainingFieldRoles() {
  const setRole = db.prepare(`UPDATE form_questions SET field_role = ? WHERE label = ? AND field_role IS NULL`);
  for (const [label, role] of [
    ["When was AED Responder training last conducted?", "training_recency"],
    ["How many people were trained in the last session?", "training_headcount"],
  ]) {
    setRole.run(role, label);
  }
}
...
// Rebalances the point system down to a clean 100-point total (it had
// drifted to 107) by bringing the two categories that had drifted from the
// campaign's own documented 100-point weights (10/15/10/10/10/15/5/10/5/10)
// back in line, and gives AEDSmartX — this campaign's own platform — a
// scoring edge over the other tracking-software options. Every UPDATE is
// matched by exact question label + option value, so a re-run is a no-op
// and an admin who has since customized these labels through the form
// builder is left alone (their question just won't match the label filter
// any more, the same trade-off every other migration here makes).
function migratePointRebalanceTo100() {
  // AED Status (was 5 + 8 = 13, target 10): "which model(s)" used to score
  // as if some listed brands were better than others, when every one is an
  // equally legitimate, commonly-serviced unit — it only forced *some*
  // selection, never graded quality. Zeroed out, with its 10-point weight
  // moved onto the AED-count question instead (a real coverage signal).
  const countQuestion = db
    .prepare(`SELECT id FROM form_questions WHERE label = 'Select the number of AEDs installed'`)
    .get();
  if (countQuestion) {
    const repointCount = db.prepare(`UPDATE form_options SET points = ? WHERE question_id = ? AND value = ?`);
    repointCount.run(6, countQuestion.id, "1");
    for (let i = 2; i <= 10; i++) repointCount.run(10, countQuestion.id, String(i));
  }
  db.prepare(
    `UPDATE form_options SET points = 0
     WHERE question_id IN (SELECT id FROM form_questions WHERE label = 'Select your AED model(s) from the options below')`
  ).run();

  // Expiry Status (was 7 + 7 = 14, target 10): same seven tiers, same
  // ordering, compressed onto a 5-point ceiling per question so battery +
  // pads expiry weigh exactly 10 together. The two most urgent tiers both
  // score 0 now — once something needs replacing this week, exactly how
  // overdue it is stops being the useful number, only "replace it" is.
  const expiryQuestions = db.prepare(`SELECT id FROM form_questions WHERE label LIKE '%expiry status%'`).all();
  const repointExpiry = db.prepare(`UPDATE form_options SET points = ? WHERE question_id = ? AND value = ?`);
  const newExpiryPoints = { expired: 0, within1w: 0, within1m: 1, within6m: 2, gt6m: 3, "1to2y": 4, gt2y: 5 };
  for (const q of expiryQuestions) {
    for (const [value, points] of Object.entries(newExpiryPoints)) {
      repointExpiry.run(points, q.id, value);
    }
  }

  // Regular Inspection (stays a 5-point ceiling): AEDSmartX now scores at
  // the ceiling with every other option strictly below it, instead of
  // tied with a third-party competitor (Plus Track).
  const trackingRepoint = db.prepare(
    `UPDATE form_options SET points = ?
     WHERE value = ? AND question_id IN (SELECT id FROM form_questions WHERE label LIKE '%track AED maintenance%')`
  );
  trackingRepoint.run(5, "aedsmartx");
  trackingRepoint.run(4, "plustrack");
  trackingRepoint.run(2, "manual");
}

// Swaps the flat "beep-yes/beep-no" SVG glyphs for real device-style
// reference graphics, on every "warning beep or error" question regardless
// of unit number. Matched by exact old image path + option value, so a
// re-run is a no-op and an admin who's already customized these is left
// alone.
function migrateBeepReferenceImages() {
  const stmt = db.prepare(`
    UPDATE form_options SET image_url = ?
    WHERE image_url = ? AND value = ?
      AND question_id IN (SELECT id FROM form_questions WHERE label LIKE '%warning beep or error message%')
  `);
  stmt.run("/reference/beep-no.jpg", "/icons/beep-no.svg", "no");
  stmt.run("/reference/beep-yes.jpg", "/icons/beep-yes.svg", "yes");
}

function migrateCabinetInaccessibleReferenceImage() {
  const stmt = db.prepare(`
    UPDATE form_options SET image_url = ?
    WHERE value = 'no'
      AND question_id IN (SELECT id FROM form_questions WHERE label LIKE '%cabinet intact and accessible%')
  `);
  stmt.run("/reference/aed-cabinet-inaccessible.jpg");
}

function migrateContactDetailsSection() {
  db.prepare(`
    UPDATE form_sections
    SET title = 'Hotel & Contact Details',
        note = NULL
    WHERE order_index = 0 OR title = 'General & Hotel Details' OR title LIKE '%General%Hotel%'
  `).run();

  const relabel = db.prepare(`UPDATE form_questions SET label = ? WHERE field_role = ?`);
  relabel.run("First name", "first_name");
  relabel.run("Last name", "last_name");
  relabel.run("Email address", "email");
  relabel.run("Phone number", "phone");
}

function migrateClearAllSectionNotes() {
  db.prepare(`UPDATE form_sections SET note = NULL`).run();
}

function migrateClearQuestionHints() {
  db.prepare(`
    UPDATE form_questions
    SET hint = NULL
    WHERE field_role IN ('pads_connected', 'readiness_indicator')
       OR hint LIKE '%pouch should lie flat%'
       OR hint LIKE '%slow-blinking green%'
       OR hint LIKE '%Example:%'
  `).run();
}

function migrateUpdateCabinetAccessibleImage() {
  db.prepare(`
    UPDATE form_options
    SET image_url = '/reference/aed-cabinet-installed.jpg'
    WHERE question_id IN (
      SELECT id FROM form_questions WHERE label LIKE '%cabinet intact and accessible%'
    ) AND value = 'yes'
  `).run();
}

// The full 200-point rework (was 100) — every change here is matched by
// exact label/value so a re-run is a no-op and an admin's own edits since
// are left alone. Order matters in a couple of places (the section rename
// must land before later steps that match sections by their NEW title),
// so read this top to bottom rather than as independent statements.
function migratePointRebalanceTo200() {
  // --- Section renames -----------------------------------------------
  db.prepare(`UPDATE form_sections SET title = 'AED Installation Status' WHERE title = 'AED Status Gate'`).run();
  db.prepare(`UPDATE form_sections SET title = 'AED Numbers and Models' WHERE title = 'AED Status'`).run();

  // --- Mark the four per-unit sections to average instead of sum ------
  const markAveraged = db.prepare(`UPDATE form_sections SET average_per_unit = 1 WHERE title = ?`);
  for (const title of ["Physical Status", "Readiness Alert on AED", "Expiry Status", "Paediatric Readiness"]) {
    markAveraged.run(title);
  }

  // --- AED count (Total 10 points: 1=6, 2-3=9, 4+=10) ------------------
  const countQuestion = db
    .prepare(`SELECT id FROM form_questions WHERE label = 'Select the number of AEDs installed'`)
    .get();
  if (countQuestion) {
    const repointCount = db.prepare(`UPDATE form_options SET points = ? WHERE question_id = ? AND value = ?`);
    repointCount.run(6, countQuestion.id, "1");
    repointCount.run(9, countQuestion.id, "2");
    repointCount.run(9, countQuestion.id, "3");
    for (let i = 4; i <= 10; i++) repointCount.run(10, countQuestion.id, String(i));
  }

  // --- AED model(s): scored per-brand now, averaged across selections -
  const modelsQuestion = db
    .prepare(`SELECT id FROM form_questions WHERE label = 'Select your AED model(s) from the options below'`)
    .get();
  if (modelsQuestion) {
    db.prepare(`UPDATE form_questions SET average_across_selections = 1 WHERE id = ?`).run(modelsQuestion.id);
    const repointModel = db.prepare(`UPDATE form_options SET points = ? WHERE question_id = ? AND value = ?`);
    const modelPoints = { frx: 10, hs1: 10, zollPlus: 10, g5: 10, defibtech: 8, crPlus: 3, lp1000: 3, other: 3 };
    for (const [value, points] of Object.entries(modelPoints)) repointModel.run(points, modelsQuestion.id, value);
  }

  // --- Serial number (1): scored from the serial's own encoded age ----
  db.prepare(`UPDATE form_questions SET score_from_serial_age = 1 WHERE field_role = 'serial_number_1'`).run();

  // --- Physical Status: 5 points each, both unit (1) and the (2) template
  const physicalRepoint = db.prepare(`
    UPDATE form_options SET points = ?
    WHERE value = ? AND question_id IN (SELECT id FROM form_questions WHERE label LIKE ?)
  `);
  physicalRepoint.run(5, "yes", "%battery%installed correctly%");
  physicalRepoint.run(5, "yes", "%pads%connected and sealed%");
  physicalRepoint.run(5, "no", "%visible damage on AED%");
  physicalRepoint.run(5, "yes", "%cabinet intact and accessible%");

  // --- Readiness Alert: green=15, no-beep=5; self-test removed entirely
  const readinessRepoint = db.prepare(`
    UPDATE form_options SET points = ?
    WHERE value = ? AND question_id IN (SELECT id FROM form_questions WHERE label LIKE ?)
  `);
  readinessRepoint.run(15, "green", "%displaying a green (ready) indicator%");
  readinessRepoint.run(5, "no", "%warning beep or error message%");
  db.prepare(`DELETE FROM form_questions WHERE label LIKE '%Self-test%passed%'`).run();

  // --- Expiry Status: >1y=15, 6mo-1y=10, 0-6mo=5, expired=0 ------------
  const expiryQuestions = db.prepare(`SELECT id FROM form_questions WHERE label LIKE '%expiry status%'`).all();
  const repointExpiry = db.prepare(`UPDATE form_options SET points = ? WHERE question_id = ? AND value = ?`);
  const newExpiryPoints = { gt2y: 15, "1to2y": 15, gt6m: 10, within6m: 5, within1m: 5, within1w: 5, expired: 0 };
  for (const q of expiryQuestions) {
    for (const [value, points] of Object.entries(newExpiryPoints)) repointExpiry.run(points, q.id, value);
  }

  // --- Paediatric Readiness: relabel unit (1), repoint, add unit (2) ---
  const paediatricSection = db.prepare(`SELECT id FROM form_sections WHERE title = 'Paediatric Readiness'`).get();
  if (paediatricSection) {
    const paediatricQ1 = db
      .prepare(`SELECT id FROM form_questions WHERE section_id = ? AND label LIKE '%Paediatric Pad or Infant/Child Key%' AND label NOT LIKE '%(2)%'`)
      .get(paediatricSection.id);
    if (paediatricQ1) {
      db.prepare(`UPDATE form_questions SET label = ? WHERE id = ? AND label NOT LIKE '%AED (%'`).run(
        "Is there a Paediatric Pad or Infant/Child Key for AED (1)?",
        paediatricQ1.id
      );
      const repointPaed = db.prepare(`UPDATE form_options SET points = ? WHERE question_id = ? AND value = ?`);
      repointPaed.run(20, paediatricQ1.id, "pads");
      repointPaed.run(20, paediatricQ1.id, "key");
      repointPaed.run(20, paediatricQ1.id, "mode");
      repointPaed.run(0, paediatricQ1.id, "no");

      const hasUnit2 = db
        .prepare(`SELECT id FROM form_questions WHERE section_id = ? AND label LIKE '%AED (2)%'`)
        .get(paediatricSection.id);
      if (!hasUnit2) {
        const q2Id = createQuestion(paediatricSection.id, {
          type: "radio",
          label: "Is there a Paediatric Pad or Infant/Child Key for AED (2)?",
          required: false,
          unscored: true,
        });
        const opts = db.prepare(`SELECT value, label, sub, image_url FROM form_options WHERE question_id = ? ORDER BY order_index`).all(paediatricQ1.id);
        const paedPoints = { pads: 20, key: 20, mode: 20, no: 0 };
        for (const o of opts) {
          createOption(q2Id, { value: o.value, label: o.label, sub: o.sub, points: paedPoints[o.value] ?? 0, imageUrl: o.image_url });
        }
      }
      // Same Railway build-vs-live race as migrateScoringColumns above, but
      // for an INSERT instead of an ALTER TABLE: two concurrent processes
      // can both see "no (2) question yet" and both create one, which
      // doesn't throw (no unique constraint on label) but does leave a
      // genuine duplicate question sitting in the section. Cleaned up
      // unconditionally on every boot rather than only guarded at create
      // time, so it also repairs any duplicate a past race already wrote.
      db.prepare(`
        DELETE FROM form_questions
        WHERE section_id = ? AND label LIKE '%AED (2)%'
        AND id NOT IN (SELECT MIN(id) FROM form_questions WHERE section_id = ? AND label LIKE '%AED (2)%')
      `).run(paediatricSection.id, paediatricSection.id);
    }
  }

  // --- AED Responder Training ------------------------------------------
  const recencyRepoint = db.prepare(`
    UPDATE form_options SET points = ?
    WHERE value = ? AND question_id IN (SELECT id FROM form_questions WHERE field_role = 'training_recency')
  `);
  recencyRepoint.run(20, "12");
  recencyRepoint.run(10, "12-24");
  recencyRepoint.run(5, "24-36");
  recencyRepoint.run(0, "over36");
  const headcountRepoint = db.prepare(`
    UPDATE form_options SET points = ?
    WHERE value = ? AND question_id IN (SELECT id FROM form_questions WHERE field_role = 'training_headcount')
  `);
  headcountRepoint.run(5, "0-15");
  headcountRepoint.run(6, "15-30");
  headcountRepoint.run(7, "30-50");
  headcountRepoint.run(8, "50-100");
  headcountRepoint.run(10, "100+");

  // --- Regular Inspection: AEDSmartX and Plus Track tied at the top ----
  const trackingRepoint2 = db.prepare(`
    UPDATE form_options SET points = ?
    WHERE value = ? AND question_id IN (SELECT id FROM form_questions WHERE label LIKE '%track AED maintenance%')
  `);
  trackingRepoint2.run(20, "aedsmartx");
  trackingRepoint2.run(20, "plustrack");
  trackingRepoint2.run(10, "manual");

  // --- Emergency Accessories & Contacts: 2 points each, 5 contacts -----
  const accessoriesRepoint = db.prepare(`
    UPDATE form_options SET points = 2
    WHERE question_id IN (SELECT id FROM form_questions WHERE label LIKE '%emergency accessories%')
  `);
  accessoriesRepoint.run();
  const contactsQuestion = db
    .prepare(`SELECT id FROM form_questions WHERE label LIKE '%emergency contact number%'`)
    .get();
  if (contactsQuestion) {
    db.prepare(`UPDATE form_options SET points = 2 WHERE question_id = ?`).run(contactsQuestion.id);
    const hasMaintenanceContact = db
      .prepare(`SELECT id FROM form_options WHERE question_id = ? AND value = 'maintenanceCompany'`)
      .get(contactsQuestion.id);
    if (!hasMaintenanceContact) {
      createOption(contactsQuestion.id, {
        value: "maintenanceCompany",
        label: "AED maintenance company",
        points: 2,
        imageUrl: "/icons/contact-maintenance.svg",
      });
    }
    // Same duplicate-insert race as the Paediatric (2) question above —
    // repairs any duplicate a past concurrent build already created, and
    // keeps this a no-op once there's exactly one.
    db.prepare(`
      DELETE FROM form_options
      WHERE question_id = ? AND value = 'maintenanceCompany'
      AND id NOT IN (SELECT MIN(id) FROM form_options WHERE question_id = ? AND value = 'maintenanceCompany')
    `).run(contactsQuestion.id, contactsQuestion.id);
  }

  // --- Documentation: 5 points each ------------------------------------
  db.prepare(`
    UPDATE form_options SET points = 5
    WHERE question_id IN (SELECT id FROM form_questions WHERE label LIKE '%documentation do you maintain%')
  `).run();

  // --- Good Samaritan: no longer contributes to the score --------------
  db.prepare(`
    UPDATE form_options SET points = 0
    WHERE question_id IN (
      SELECT id FROM form_questions WHERE section_id IN (
        SELECT id FROM form_sections WHERE title LIKE '%Good Samaritan%'
      )
    )
  `).run();
}

// The seed data has always numbered "AED Installation Status" and "AED
// Numbers and Models" both "2" (a copy-paste slip — Hotel & Contact
// Details is "1", so the next two sections should read "2" and "3" before
// the acronym sections take over at "P"), and every already-seeded DB
// carries that same duplicate. Matched by exact title, so an admin who has
// since renamed either section keeps whatever letter they set.
function migrateFixDuplicateSectionLetter() {
  db.prepare(`UPDATE form_sections SET letter = '3' WHERE title = 'AED Numbers and Models' AND letter = '2'`).run();
}

// A bare "1"/"2"/"3" numeral (the three sections before the PREPARED
// acronym sections take over at "P") read as a stray, unstyled digit next
// to everything else in the same badge — a trailing period matches how a
// numbered list item normally reads and makes clear it's a step number,
// not a data value. Matched by exact (title, current letter) pair, so it's
// a no-op once already applied and never overwrites an admin's own edit.
function migrateNumberedSectionLetters() {
  const stmt = db.prepare(`UPDATE form_sections SET letter = ? WHERE title = ? AND letter = ?`);
  stmt.run("1.", "Hotel & Contact Details", "1");
  stmt.run("2.", "AED Installation Status", "2");
  stmt.run("3.", "AED Numbers and Models", "3");
}

// The prize wheel (see lib/prizes.js, components/PrizeWheel.jsx) picks a
// winner at submission time but never collected a shipping address for the
// physical prizes — this adds the columns for it. Delivery details are
// saved separately, after the audit itself is already submitted (see
// saveDeliveryAddress + app/api/submissions/[id]/delivery/route.js), which
// is why these all default NULL rather than being part of the original
// INSERT in insertSubmission.
function migrateDeliveryColumns() {
  addColumnIfMissing("submissions", "delivery_name", `ALTER TABLE submissions ADD COLUMN delivery_name TEXT`);
  addColumnIfMissing("submissions", "delivery_address1", `ALTER TABLE submissions ADD COLUMN delivery_address1 TEXT`);
  addColumnIfMissing("submissions", "delivery_address2", `ALTER TABLE submissions ADD COLUMN delivery_address2 TEXT`);
  addColumnIfMissing("submissions", "delivery_city", `ALTER TABLE submissions ADD COLUMN delivery_city TEXT`);
  addColumnIfMissing("submissions", "delivery_state", `ALTER TABLE submissions ADD COLUMN delivery_state TEXT`);
  addColumnIfMissing("submissions", "delivery_postal_code", `ALTER TABLE submissions ADD COLUMN delivery_postal_code TEXT`);
  addColumnIfMissing("submissions", "delivery_country", `ALTER TABLE submissions ADD COLUMN delivery_country TEXT`);
  addColumnIfMissing("submissions", "delivery_notes", `ALTER TABLE submissions ADD COLUMN delivery_notes TEXT`);
  addColumnIfMissing("submissions", "delivery_submitted_at", `ALTER TABLE submissions ADD COLUMN delivery_submitted_at TEXT`);
}

// certificate_number is assigned once, in submission order, to every
// Good Samaritan Warrior-qualifying report — see formatCertificateNumber
// in lib/certificateNumber.js and the assignment in insertSubmission
// below. This backfills every submission that existed before this column
// did, in the same chronological order they'd have been numbered in had
// this existed from the start, so the running count genuinely reflects
// "how many of these have ever been issued" rather than starting over at
// whatever moment this migration happens to run.
function migrateCertificateNumbers() {
  addColumnIfMissing("submissions", "certificate_number", `ALTER TABLE submissions ADD COLUMN certificate_number INTEGER`);

  const alreadyNumbered = db.prepare(`SELECT COUNT(*) AS c FROM submissions WHERE certificate_number IS NOT NULL`).get().c;
  if (alreadyNumbered > 0) return; // backfill already ran — new submissions get their number at insert time instead

  const schema = getFormSchema();
  if (schema.sections.length === 0) return; // fresh install, nothing seeded yet — nothing to backfill either

  const subs = db.prepare(`SELECT id, answers_json FROM submissions ORDER BY created_at ASC`).all();
  const update = db.prepare(`UPDATE submissions SET certificate_number = ? WHERE id = ?`);
  let sequence = 0;
  for (const sub of subs) {
    ...
    if (scored.qualifiesForCertificate) {
      sequence++;
      update.run(sequence, sub.id);
    }
  }
}

// "AED maintenance company" wrapped onto three lines in its option card,
// where every sibling contact ("Nearest hospital", "Doctor on call") fits on
// one or two — "AED service provider" says the same thing and sits on the
// card the way the others do. Matched on the option's value, so an admin who
// has since renamed it through the form builder keeps their wording.
function migrateServiceProviderLabel() {
  db.prepare(
    `UPDATE form_options SET label = 'AED service provider'
     WHERE value = 'maintenanceCompany' AND label = 'AED maintenance company'`
  ).run();
}
...
function seedFormIfEmpty() {
  ...
  const insertSection = db.prepare(
    `INSERT INTO form_sections (order_index, letter, title, note, is_supplementary, unscored, average_per_unit, visible_if_question_id, visible_if_value)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  );
  const insertQuestion = db.prepare(
    `INSERT INTO form_questions (section_id, order_index, type, label, hint, required, unscored, average_across_selections, score_from_serial_age, max_selections, max_selections_linked_question_id, derived_from_date_question_id, field_role, gate_role, gate_value, scale_min, scale_max, scale_min_label, scale_max_label)
     VALUES (@sectionId, @orderIndex, @type, @label, @hint, @required, @unscored, @averageAcrossSelections, @scoreFromSerialAge, @maxSelections, @maxSelectionsLinkedQuestionId, @derivedFromDateQuestionId, @fieldRole, @gateRole, @gateValue, @scaleMin, @scaleMax, @scaleMinLabel, @scaleMaxLabel)`
  );
    ...
    sections.forEach((section, sIdx) => {
      ...
      (section.questions || []).forEach((question, qIdx) => {
        const qResult = insertQuestion.run({
          ...
          averageAcrossSelections: question.averageAcrossSelections ? 1 : 0,
          scoreFromSerialAge: question.scoreFromSerialAge ? 1 : 0,
...
export function insertSubmission({ answers, scored, identity, prize }) {
  ...
  // Assigned as one atomic scalar subquery inside the same INSERT, not a
  // separate SELECT beforehand — two concurrent submissions could otherwise
  // both read the same current MAX() before either one's INSERT commits
  // and land on the same number. A subquery inside the INSERT statement
  // runs as part of that single synchronous better-sqlite3 call, with no
  // point where another request's JS could interleave and read a stale
  // MAX() in between.
  const certificateNumberExpr = scored.qualifiesForCertificate
    ? `(SELECT COALESCE(MAX(certificate_number), 0) + 1 FROM submissions)`
    : "NULL";
  db.prepare(
    `INSERT INTO submissions
      (id, created_at, hotel, first_name, last_name, email, phone, has_aed,
       total_points, total_max, prepared_points, prepared_max, answers_json, prize, certificate_number)
     VALUES (@id, @createdAt, @hotel, @firstName, @lastName, @email, @phone, @hasAED,
       @totalPoints, @totalMax, @preparedPoints, @preparedMax, @answersJson, @prize, ${certificateNumberExpr})`
  ).run({
    id,
    createdAt,
...
// Saves (or overwrites, if the winner comes back to their report link and
// edits it) the shipping address for a prize-winning submission. Returns
// false when the id doesn't exist at all, so the API route can 404 rather
// than silently succeed on a made-up id.
export function saveDeliveryAddress(id, { name, address1, address2, city, state, postalCode, country, notes }) {
  const result = db
    .prepare(
