...
if (!db) {
  ...
  // volume the *currently live* deployment already has open. WAL mode
  // allows concurrent readers, but without a busy timeout any write-lock
  // contention between the two (e.g. the build starting mid-way through a
  // live submission) throws SQLITE_BUSY immediately instead of just
  // waiting the near-instant moment it takes to clear — which is what
  // failed two Railway builds outright (2026-08-04 and 2026-08-05). This
  // module now also runs schema migrations at import time (see
  // migrateSubmissionsTable below), which hold a write lock for longer than
  // a plain read — raised from 5000ms to 15000ms so a same-moment build vs.
  // live-deploy collision has more room to just wait it out instead of
