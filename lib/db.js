...
function migrateScoringColumns() {
  const sectionColumns = db.prepare(`PRAGMA table_info(form_sections)`).all();
  if (!sectionColumns.some((c) => c.name === "average_per_unit")) {
    db.exec(`ALTER TABLE form_sections ADD COLUMN average_per_unit INTEGER NOT NULL DEFAULT 0`);
  }
  const questionColumns = db.prepare(`PRAGMA table_info(form_questions)`).all();
  if (!questionColumns.some((c) => c.name === "average_across_selections")) {
    db.exec(`ALTER TABLE form_questions ADD COLUMN average_across_selections INTEGER NOT NULL DEFAULT 0`);
  }
  if (!questionColumns.some((c) => c.name === "score_from_serial_age")) {
    db.exec(`ALTER TABLE form_questions ADD COLUMN score_from_serial_age INTEGER NOT NULL DEFAULT 0`);
  }
}
