...
import { isPlausibleExpiry, expiryCrossCheckAgrees, parseExpiryDate } from "./inspectionValidators";
...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    // Only meaningful for a multi-frame (video) call — which of the
    // submitted frames actually shows the evidence for this verdict. Used
    // server-side to persist THAT frame as the audit's visual record
    // instead of always saving frame 1, which previously could show a
    // dark/inconclusive moment even when the verdict was correctly based
    // on a flash caught later in the clip.
    key_frame_index: { type: Type.INTEGER, nullable: true },
  },
...
function buildPrompt(item, model, frameCount) {
  const sequenceNote = frameCount
    ? `You are given ${frameCount} still frames extracted evenly across ` +
      ...
      "Also set key_frame_index to the 1-based position (1.." +
      frameCount +
      ") of the single frame that most clearly shows your evidence — " +
      "e.g. the frame the flash is visible in, or simply the sharpest, " +
      "most in-focus frame if nothing decisive stands out. This is used " +
      "to pick which exact frame gets saved as the audit record, so it " +
      "should genuinely be the most representative one, not always frame " +
      "1.\n\n"
    : "";
  return (
...
// RESOURCE_EXHAUSTED covers two very different situations that isRetryable
// above can't tell apart from the status code alone: a short-lived
// per-minute rate limit (worth retrying — it clears in seconds) and a hard
// daily/monthly quota or billing cap (won't clear no matter how long this
// one request waits). Gemini's own error message spells out the latter
// with the word "quota" specifically, so a hit here skips the backoff
// loop entirely — retrying 3 times against an exhausted quota only costs
// every concurrent auto-scanning user up to ~3x the per-model timeout for
// a failure that was never going to succeed.
function isHardQuotaExhaustion(err) {
  return /\bquota\b/i.test(String(err?.message || ""));
}
...
async function withRetry(fn, attempts = 3) {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      lastErr = err;
      if (i === attempts - 1 || isHardQuotaExhaustion(err) || !isRetryable(err)) throw err;
...
function applyDeterministicChecks(item, result) {
  // Contract-compliance floor, applied to every item (not just the two
  // expiry-date ones above): the prompt requires a short explanatory
  // sentence for every verdict. A response that skips it is a sign of a
  // malformed or truncated read, not a genuinely confident one — even if
  // Gemini's own confidence score claims otherwise. Clamping confidence
  // here (rather than forcing passed=false) lets the existing low-
  // confidence handling in components/AutoInspection.jsx route it to
  // manual review without this file needing to know about that UI layer.
  if (!result.notes || !result.notes.trim()) {
    return {
      ...result,
      confidence: Math.min(result.confidence ?? 0, 0.4),
