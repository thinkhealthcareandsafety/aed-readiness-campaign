...
// (required, scored) and "(2)" (optional inventory). This adapts that to
// whatever the responder actually reported: exactly 1 unit drops the "(2)"
// block entirely (nothing to ask about a unit that doesn't exist); exactly 2
// leaves the static blocks as they were seeded; 3+ clones the "(2)" block as
// many extra times as needed, renumbering labels and re-linking each clone's
// derived-date question to its own sibling clone (not the original "(2)"
// one). Clones are purely in-memory — nothing is written back to the
// form_questions table — and inherit the "(2)" block's required/unscored
// flags, so every generated unit stays optional inventory exactly like unit
// 2 always was.
    ...
    sections: schema.sections.map((section) => {
      ...
      for (let unit = 3; unit <= count; unit++) {
        ...
        const clones = template.map((q) => {
          ...
          return { ...q, id: cloneId, label: q.label.replace("(2)", `(${unit})`) };
        });
        clones.forEach((c) => {
...
export function validateAnswers(schema, answers) {
  for (const section of schema.sections) {
    if (!isSectionVisible(section, answers)) continue;
    const err = validateSection(section, answers);
    if (err) return err;
  }
  return null;
}
