...
// property-specific logo set in lib/hotelPropertyLogos.js first, which now
// covers every hotel in lib/hotels.js. This tier exists purely so a future
// hotel added there before its own logo is sourced still gets at least a
// recognizable brand mark instead of nothing. A generic per-brand mark was
// previously the ONLY tier, which is why every Courtyard property (for
// example) used to show the identical logo — this module's own logos are
// Wikimedia Commons brand marks (see public/hotel-logos/). Two brands in
// the hotel list — The Ritz-Carlton and Tribute Portfolio — don't have a
// clean, properly-licensed generic brand wordmark on Commons (only
// property-specific ones), so they intentionally have no logo here and
// fall back to text-only if their real logo is ever missing too.
const BRANDS = [
