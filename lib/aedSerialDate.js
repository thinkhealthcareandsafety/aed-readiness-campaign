...
// Just the worked example (no prose), for use as an input placeholder shown
// before the responder has typed anything — the full FORMAT_HINTS prose is
// for the after-the-fact "that doesn't match" error, this is the up-front nudge.
...
export function aedSerialExample(model) {
  return SERIAL_EXAMPLES[model] || null;
}
...
//
// ageYears is deliberately a plain calendar-year difference (today's year
// minus manufacture year), not a precise day count — the UI only ever
// displays the year, never the month, so two units manufactured in the
// same year (e.g. March 2020 and December 2020) must land on the exact
// same age/tier. A day-precise diff would silently split them onto
// different sides of a tier boundary purely from the hidden month, which
// reads as an outright bug ("both say 2020, why is one worse?") rather
// than the fine-grained accuracy it was meant to be.
export function aedAgeStatus(model, serial, today = new Date()) {
  ...
  const ageYears = today.getFullYear() - decoded.year;
