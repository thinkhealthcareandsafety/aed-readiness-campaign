...
// fitFontSize floors out at minSize regardless of whether the text
// actually fits there — a long enough double-barrelled name (or an
// honorific/suffix typed into the name fields) could still overflow past
// the green rule onto the rest of the artwork at the smallest size this
// script face stays legible at. Rather than let that render silently
// broken, this clips with an ellipsis the same way any other UI would.
function truncateToFit(font, text, maxWidth, size) {
  if (inkWidth(font, text, size) <= maxWidth) return text;
  let truncated = text;
  while (truncated.length > 1 && inkWidth(font, `${truncated}…`, size) > maxWidth) {
...
export function renderCertificate({ name, certificateNumber }) {
  ...
  const fittedName = truncateToFit(scriptFont, displayName, NAME.maxWidth, nameSize);
  const namePath = placedPath(scriptFont, fittedName, nameSize, NAME.baselineY, { centerX: NAME.centerX });
