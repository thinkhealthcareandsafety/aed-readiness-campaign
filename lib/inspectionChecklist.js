...
export const CHECKLIST_ITEMS = [
  ...
  // aed_cabinet_ok and readiness_indicator used to be auto-inspected here
  // too (a photo and a short video respectively) — dropped from this list
  ...
  // it, and the readiness-indicator video was the single most error-prone
  // capture in the whole flow (see the retired prompt's own "single most
  // common mistake" framing) — a short clip missing the slow blink cycle
  // routed to manual review often enough that asking for it up front no
  // longer earned its place. Both questions still get answered — just
  // manually, in their normal spot in Physical Status / Readiness Alert,
  // with the same real reference photos those questions already show
  // (aed-cabinet-example.jpg and the per-model ready/not-ready photos via
  // MODEL_PHOTOS) — not skipped or left blank. A task list built from this
  // array (see buildTasks in AutoInspection.jsx) simply never generates a
  // capture step for a concept that isn't listed here, and any question
  // without an auto-inspection result already falls through to the normal
  // manual QuestionBlock, exactly like an "unclear"/skipped result does
  // today for any other item.
];
